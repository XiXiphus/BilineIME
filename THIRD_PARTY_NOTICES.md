...
| `librime-octagram` | <https://github.com/lotem/librime-octagram> | GPL-3.0 | Rime language-model plugin for sentence ranking and contextual suggestions | Vendored under `Vendor/librime/plugins/octagram`; merged into the bundled `librime.1.dylib` |
| `librime-predict` | <https://github.com/rime/librime-predict> | BSD-3-Clause | Rime next-word prediction plugin | Vendored under `Vendor/librime/plugins/predict`; merged into the bundled `librime.1.dylib` |
| `rime-octagram-data` | <https://github.com/lotem/rime-octagram-data> | LGPL-3.0 | Official Rime grammar configuration and simplified/traditional language-model data | Vendored under `Vendor/rime-octagram-data`; selected `.gram` models and `grammar.yaml` are bundled in app runtime |
| Biline minimal predict data | local project data | GPL-3.0 | Small smoke-test prediction data for `librime-predict` until a benchmarked model is selected | Vendored under `Vendor/rime-predict-data`; compiled into `predict.db` during Rime runtime build |
| UTF8-CPP | <https://github.com/nemtrif/utfcpp> | BSL-1.0 | Header-only UTF-8 helpers required by `librime-octagram` | Selected headers vendored under `Vendor/librime/include` |
| Darts-clone | <https://github.com/s-yata/darts-clone> | BSD-style; see bundled notice | Header-only double-array trie used by `librime-predict` | Selected header and notice vendored under `Vendor/librime/include` |
